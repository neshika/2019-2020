...
<body>
  <div class="wrapper">
    <form action="crud.php" method="POST">
      ...
      /* проверка: если нажата кнопка и внеснено название предмета(ИТЕМ)*/
      if (isset($_POST['FndItem']) and (!empty($_POST['retItemName']))) {
        $id_item = $itm->retItemIdByName($_POST['retItemName']);
        if (!empty($id_item)) {
          $_POST['retItemId'] = $id_item; //вносятся данные в переменную пост, для следующе проверки
        }
      }
      /* проверка: если нажата кнопка найти и введен ИД Итем или внесен в предыдущей проверке*/
      if (isset($_POST['FndItem']) and (!empty($_POST['retItemId']))) {
       $str_item = $tbl->retRow($_POST['retItemId'],'items');
        if(!empty($str_item)) {
          $itm->dataItem($str_item); //выводятся данные, если ИТЕМ найден
        }
      }
      ...
      <?php
      /* проверка: если кнопка найти нажата и внесено название рецепта*/
